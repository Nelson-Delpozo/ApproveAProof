...
describe("createOrganizationSlug", () => {
  it("converts an organization name to a lowercase slug", () => {
    expect(createOrganizationSlug("Acme Printing")).toBe(
      "acme-printing",
    );
  });
  ...
  it("replaces punctuation and spaces with hyphens", () => {
    expect(createOrganizationSlug("Nelson's Print Shop")).toBe(
      "nelson-s-print-shop",
    );
  });

  it("removes diacritical marks", () => {
    expect(createOrganizationSlug("José's Signs")).toBe(
      "jose-s-signs",
    );
  });

  it("collapses consecutive separators", () => {
    expect(createOrganizationSlug("Acme --- Print & Design")).toBe(
      "acme-print-design",
    );
  });
